...
void estimator::gyr_callback(const geometry_msgs::Vector3Stamped::ConstPtr &msg) //
{
  if (!imu_init)
  {
    imu_init = 1;
...
estimator::estimator()
{
  ...
  ros::Subscriber gyr_listener = nh_imu.subscribe("/imu/angular_velocity", 1, &estimator::gyr_callback, this);        // gyr回调
